...
# ── External-factor (weather) sensitivity of the energy-aware modes ──────────
# EXOG_EFFECT_SCALE damps how strongly the ef_* external factors move the
# energy-aware simulation. Each ef_* value handed to a curve pipeline or written
# into the energy state is shrunk toward the reference level of that column --
# its mean over all activities, i.e. "average weather" for this process:
#
#     ef_used = ref + EXOG_EFFECT_SCALE * (ef_activity - ref)
#
# 1.0 reproduces the previous behaviour exactly (no damping); 0.0 removes the
# weather sensitivity entirely and predicts at average conditions. Values in
# between keep the direction of the effect and reduce its magnitude, which is
# what this knob is for: the curve models are fitted per (sensor, activity,
# object) on few instances, so an ef_* coefficient can pick up more of the
# between-activity variation than the physics supports, and the simulated
# profiles then swing with the weather more than the real ones do.
#
# NOTE this affects the energy-aware simulation modes only. The evaluations
# that predict curves after the fact (complete-curve, schedule-profile) read
# their external factors from the real ef_* series through
# sim_extractor.build_exog_lookup and do not pass through here.
EXOG_EFFECT_SCALE = 0.5
...
class ProcessSimulation:
    ...
    def __init__(self, activity_stats_df, production_plan,
        ...
        # Reference level each ef_* column is shrunk toward (see
        # EXOG_EFFECT_SCALE / _shrink_exog): the mean of that column over all
        # activities, i.e. "average weather" for this process.
        self._exog_reference_means        = self._compute_exog_reference_means()
    ...
    def _compute_exog_reference_means(self):
        """
        Reference level per ef_* column: its mean across all activities.

        This is the "average weather" the per-activity values are shrunk toward
        by _shrink_exog. Computed once from activity_exog_means, which is itself
        a per-activity training mean, so no test data is touched.
        """
        if not self.activity_exog_means:
            return {}
        acc = defaultdict(list)
        for _act_means in self.activity_exog_means.values():
            for col, val in (_act_means or {}).items():
                try:
                    acc[col].append(float(val))
                except (TypeError, ValueError):
                    continue
        return {col: float(np.mean(vals)) for col, vals in acc.items() if vals}

    def _shrink_exog(self, exog_mapping):
        """
        Damp the external factors toward average conditions (EXOG_EFFECT_SCALE).

        Returns a new {col: value} dict with each ef_* value replaced by
        ref + scale * (value - ref), where ref is the column's mean over all
        activities. Columns without a reference, non-numeric values and
        scale == 1.0 are passed through untouched.
        """
        if not exog_mapping or EXOG_EFFECT_SCALE == 1.0:
            return dict(exog_mapping or {})
        out = {}
        for col, val in exog_mapping.items():
            ref = self._exog_reference_means.get(col)
            if ref is None:
                out[col] = val
                continue
            try:
                out[col] = ref + EXOG_EFFECT_SCALE * (float(val) - ref)
            except (TypeError, ValueError):
                out[col] = val
        return out
        ...
        for _, obj_config in unique_objects.iterrows():
            ...
            while step < max_steps:
                ...
                if self.energy_pipelines and chosen_label is not None:
                    ...
                    for sensor, act_map in self.energy_pipelines.items():
                        try:
                            ...
                            if ep.get('exog_cols') and self.activity_exog_means:
                                # Damped toward average conditions — see
                                # EXOG_EFFECT_SCALE.
                                _act_means = self._shrink_exog(
                                    self.activity_exog_means.get(chosen_label, {})
                                )
                                _exog_vals = {
                                    col: np.array([v])
                    ...
                    # damped toward average conditions (EXOG_EFFECT_SCALE) so the
                    # duration/transition modifiers see the same weather signal
                    # the curve pipelines above were given.
                    if self.activity_exog_means and chosen_label in self.activity_exog_means:
                        new_energy_state.update(
                            self._shrink_exog(self.activity_exog_means[chosen_label])
                        )
