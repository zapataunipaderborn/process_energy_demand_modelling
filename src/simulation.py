...
class ProcessSimulation:
    ...
    mode : {'statistical', 'ml', 'petri_net'}
        'statistical' – sample durations/transitions from fitted
        distributions (or normal as fallback).
        'ml'          – use ML models from *ml_models* for both duration
        and transitions; falls back to statistical per activity when no
        model is available.
    ...
    def __init__(self, activity_stats_df, production_plan,
                 mode='statistical', ml_models=None,
                 process_models=None, energy_pipelines=None,
        ...
        if self.mode == 'ml' and self.ml_models is None:
            print(f"[ProcessSimulation] WARNING: mode='ml' but no ml_models "
        ...
        if self.mode == 'petri_net' \
            ...
            print(f"[ProcessSimulation] WARNING: mode='petri_net' but no "
    ...
    def _get_activity_duration(self, activity, object_name, object_type,
                               ...
                               activity_index=0):
        """
        Sample the duration for one activity instance.

        In 'ml' mode the ML duration model is tried first; it falls back
        to the statistical path when no model exists for this key.
        In 'statistical' / 'petri_net' mode the best-fit distribution stored
        in activity_stats is used directly.
        """
    ...
    def _get_next_activity(self, current_activity, object_name, object_type,
        ...
        In 'ml' mode the ML transition classifier is tried first; it
        falls back to the statistical (frequency-based) path automatically.
        In 'statistical' / 'petri_net' mode, transitions always come from
        the statistical (frequency-based) path.
        """
        ...
        # Statistical path (used by 'statistical', and as fallback for 'ml' mode)
        ...
        for _, obj_config in unique_objects.iterrows():
            ...
            while step < max_steps:
                ...
                activity_duration = self._get_activity_duration(
                    chosen_label, object_name, object_type,
                    higher_level_activity, object_attributes,
                    activity_history=activity_history[-2:] if activity_history else None,
                    activity_index=activity_count,
                )
    ...
    def _simulate_process_for_case(self, case_id, object_attributes, start_time):
        ...
        for _, obj_config in unique_objects.iterrows():
            ...
            # Reset energy state for new case
            self.current_energy_state = {}
            activity_count = 0
            ...
            while current_activity:
                ...
                # Combine static attributes with dynamic energy state features
                features = {**object_attributes, **self.current_energy_state}

                # Get duration
                activity_duration = self._get_activity_duration(
                    current_activity, object_name, object_type,
                    higher_level_activity, features,
                    activity_history=activity_history,
                    activity_index=activity_count,
                )
                ...
                # ── Update energy state for the NEXT activity ─────────────────
                if self.energy_pipelines:
                    self._update_energy_state(current_activity, object_attributes)
                    # CRITICAL FIX: Refresh features so next_activity sees the updated energy_prev_*
                    features.update(self.current_energy_state)
    ...
    def _update_energy_state(self, activity, object_attributes):
        """
        Predict energy curves for the just-completed activity and update
        self.current_energy_state with 'energy_prev_*' summary features.
        """
        ...
        for sensor, ep in self.energy_pipelines.items():
            try:
                ...
                if ref_curve is not None:
                    # Predict curve based on activity and fixed attributes
                    curve = (predict_fn(
                    ...
                    # Log the curve for validation
                    if self.events and self.events[-1]['activity'] == activity:
                    ...
                    # Summarize to scalar features
                    summary = _energy_summary(curve, sensor)
                    # Prefix with 'energy_prev_' to match training feature names
                    for k, v in summary.items():
                        new_state[f'energy_prev_{k}'] = v
            except Exception as exc:
                print(f"    [!] Energy prediction failed for {sensor}: {exc}")
        
        self.current_energy_state = new_state
