...
1. **duration_models**     – regressor predicting duration (main prediction).
...
Available model types (configured via *model_types* list):
    ...
    'mean'     – constant mean predictor (baseline)
    'median'   – constant median predictor (baseline)

All models in the list are trained for each key, and the **best one on the
internal validation set** is selected automatically. The winning model is then
re-trained on all incoming data.

Feature columns accepted:
    attr_*          – flattened object_attributes dict entries
    prev_activity_1/2  – label-encoded previous activities
    prev_duration_1/2  – previous durations in minutes
    <sensor>_mean/end/std – energy state features (when ml_with_energy mode)
...
class _ConstantPredictor:
    ...
    For regression:
    - strategy='mean'   → training mean.
    - strategy='median' → training median.
    
    For classification:
    - strategy='mean' or 'median' → training mode (most frequent label).
    """
    ...
    def fit(self, X, y):
        ...
        # Classification detection: integers from LabelEncoder
        if np.issubdtype(y_arr.dtype, np.integer):
            self.classes_ = np.unique(y_arr)
            # For classification baseline, use Mode regardless of mean/median
            from collections import Counter
            counts = Counter(y_arr)
            self._value = counts.most_common(1)[0][0]
        else:
            # Regression
    ...
    def predict_proba(self, X):
        ...
        if self.classes_ is None:
            raise AttributeError("predict_proba called but model was not fitted on classification (integer) data.")
        
        n_classes = len(self.classes_)
        # Find index of the constant value in sorted classes
        try:
            val_idx = np.where(self.classes_ == self._value)[0][0]
        except IndexError:
            val_idx = 0
    ...
    def __repr__(self):
        ctx = "Classification" if self.classes_ is not None else "Regression"
        return f"_ConstantPredictor({ctx}, strategy='{self.strategy}', value={self._value})"
...
class SimModeller:
    ...
    def predict_duration(
        ...
        If ``activity_history`` is supplied (list of ``(activity_name, duration_min)``
        tuples, most-recent first), the ``prev_activity_*`` and
        ``prev_duration_*`` feature columns are populated from it.
    ...
    def _attrs_to_features(object_attributes: dict,
        ...
        """
        Map dictionary of attributes (static or dynamic energy state) 
        to the model's feature space.
        """
        feats = {}
        for col in feature_cols:
            if col in object_attributes:
                # Direct match (e.g. 'energy_prev_mean')
                feats[col] = object_attributes[col]
            elif col.startswith('attr_'):
                # Legacy attribute mapping
                feats[col] = object_attributes.get(col[5:], 0)
            else:
                # Default for prev_* or missing columns
                feats[col] = 0
        return feats
