...
ENERGY_PARAMS = {
    ...
    # Autoclave steam: base + beta*(T_ref - T)  [colder → more wall heat loss]
    'autoclave_steam': {
        'base_kW':   200.0,
        'T_ref':      15.0,   # reference temperature °C
        'beta':        0.8,   # kW per °C below reference
        'noise_std':   4.0,
    },
    # Autoclave cooling water: base + gamma*T  [warmer → more cooling needed]
    'autoclave_cooling': {
        'base_kW':   150.0,
        'gamma':       2.5,   # kW per °C ambient
        'noise_std':   5.0,
    },
...
def _power_curve(activity: str, station: str, duration_min: float,
    ...
    if station == 'Autoclaving':
        if activity == 'heat':
            p = ENERGY_PARAMS['autoclave_steam']
            level = p['base_kW'] + p['beta'] * max(0.0, p['T_ref'] - T)
            # Ramp up over first half, full power second half
            profile = level * np.clip(t * 2, 0, 1)
            return profile + rng.normal(0, p['noise_std'], n)
        ...
        if activity == 'cool':
            p = ENERGY_PARAMS['autoclave_cooling']
            level = p['base_kW'] + p['gamma'] * T
            level = max(30.0, level)
            # Ramp up then exponential decay as vessel cools
            profile = level * 4 * t * np.exp(-3 * t)
            return profile + rng.normal(0, p['noise_std'], n)
